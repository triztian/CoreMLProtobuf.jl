...
function meta(::Type{FileOptions})
    ProtoBuf.metalock() do
        if !isassigned(__meta_FileOptions)
            ...
            val = Dict{Symbol,Any}(:java_multiple_files => false, :java_string_check_utf8 => false, :optimize_for => FileOptions_OptimizeMode.SPEED, :cc_generic_services => false, :java_generic_services => false, :py_generic_services => false, :deprecated => false, :cc_enable_arenas => false)
            fnum = Int[1,8,10,20,27,9,11,16,17,18,23,31,36,37,39,40,999]
            allflds = Pair{Symbol,Union{Type,String}}[:java_package => AbstractString, :java_outer_classname => AbstractString, :java_multiple_files => Bool, :java_generate_equals_and_hash => Bool, :java_string_check_utf8 => Bool, :optimize_for => Int32, :go_package => AbstractString, :cc_generic_services => Bool, :java_generic_services => Bool, :py_generic_services => Bool, :deprecated => Bool, :cc_enable_arenas => Bool, :objc_class_prefix => AbstractString, :csharp_namespace => AbstractString, :swift_prefix => AbstractString, :php_class_prefix => AbstractString, :uninterpreted_option => Base.Vector{UninterpretedOption}]
...
function meta(::Type{EnumDescriptorProto})
    ProtoBuf.metalock() do
        if !isassigned(__meta_EnumDescriptorProto)
            ...
            allflds = Pair{Symbol,Union{Type,String}}[:name => AbstractString, :value => Base.Vector{EnumValueDescriptorProto}, :options => EnumOptions]
